"""
This script performs object detection on a YouTube livestream to detect cars.
It uses a TensorFlow model to process frames from the livestream and sends a notification
when a car is detected. The detected frames and a 10-second video clip are saved locally.
...
class ObjectDetection:
    ...
    def notify_car_detection(self):
        """
        Send a notification to the specified URL when a car is detected.
        """
        try:
    ...
    def detect_car(self, frame):
        """
        Detect cars in the given frame using the TensorFlow model.
        Returns a tuple indicating whether a car was detected and the detection details.
        """
        ...
        for i in range(len(detection_boxes)):
            if detection_classes[i] == 3 and detection_scores[i] > 0.7: # 3 = car, check github readme for other classes.
                return True, detection_boxes, detection_classes, detection_scores
        return False, None, None, None

    def process_frame(self, frame):
        """
        Process a single frame to detect cars, send notifications, and save detected frames and video clips.
        """
        detected, boxes, classes, scores = self.detect_car(frame)
        if detected:
            threading.Thread(target=self.notify_car_detection, daemon=True).start()
            ...
            video_writer = cv2.VideoWriter(f'recorded_media/saved_car_clip/car_clip_{timestamp}.avi', fourcc, 30.0, (frame.shape[1], frame.shape[0]))
    ...
    def capture_frames(self):
        ...
        # Record the time of the last car detection
...
if __name__ == "__main__":
    ...
    notification_url = "http://127.0.0.1:8080/sendVehicleDetectionEmail"
