...
static esp_err_t control_flash_light(httpd_req_t *req) {
  ...
  int control = atoi(_control);
  int value = atoi(_value);
  flash_light(control, value);

  char response[256];
  snprintf(response, sizeof(response), "Flash Light control successfully applied.\nControl [ on /off ] : %d\nValue [ intensity ]: %d %", control, value);
  httpd_resp_send(req, response, HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}

static esp_err_t control_servo_motors(httpd_req_t *req) {
  char *buf = NULL;
  char _control[32];
  char _value[32];
  if (parse_get(req, &buf) != ESP_OK) {
  ...
  int control = atoi(_control);
  int value = atoi(_value);
  // moveSelectedServo(int servoId, int angle)
  moveSelectedServo(control, value);

  char response[256];
  snprintf(response, sizeof(response), "Servo Motors control successfully applied.\nControl [ servo id ] : %d\nValue [ rotate angle ]: %d degree", control, value);
  httpd_resp_send(req, response, HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}
...
void startCameraServer() {
  ...
  httpd_uri_t servo_motors_uri = {
    .uri = "/servo_motors",
    .method = HTTP_GET,
    .handler = control_servo_motors,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  httpd_uri_t flash_light_uri = {
    .uri = "/flash_light",
    .method = HTTP_GET,
    .handler = control_flash_light,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &servo_motors_uri);
    httpd_register_uri_handler(camera_httpd, &flash_light_uri);
  }
